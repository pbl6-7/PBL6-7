...
export const registerForActivity = (data: RegistrationRequest) =>
  apiClient.post<ApiResponse<RegistrationResponse>>('/registrations', data);

/** 获取我的报名记录 */
export const getMyRegistrations = (page = 1, size = 10) =>
  apiClient.get<ApiResponse<RegistrationPageResponse>>('/registrations/my', { params: { page, size } });

/** 获取活动报名列表 */
export const getActivityRegistrations = (activityId: number, page = 1, size = 10) =>
  apiClient.get<ApiResponse<RegistrationPageResponse>>(`/registrations/activity/${activityId}`, { params: { page, size } });

/** 更新报名状态 */
export const updateRegistrationStatus = (data: RegistrationStatusUpdateRequest) =>
  apiClient.put<ApiResponse<RegistrationResponse>>('/registrations/status', data);

/** 取消报名 */
export const cancelRegistration = (activityId: number) =>
  apiClient.delete<ApiResponse<void>>(`/registrations/activity/${activityId}`);
