...
export const publishComment = (activityId: number, data: CommentRequest) =>
  apiClient.post<ApiResponse<CommentResponse>>(`/activities/${activityId}/comments`, data);

/** 获取评论列表 */
export const getComments = (activityId: number, page?: number, size?: number) =>
  apiClient.get<ApiResponse<CommentResponse[]>>(`/activities/${activityId}/comments`, { params: { page, size } });
